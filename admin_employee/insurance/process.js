...
$(document).ready(function() {
  ...
  fillgov();
	...
	dataRecords = $('#insuranceList').DataTable({
		//dom: 'lrtip',\
		dom: 'Blfrtip',
        buttons: [
		...
		"order": [],
		"ajax": {
			url: "insurance/process.php",
			type: "POST",
			data: {
				action: 'listinsurance'
			},
			dataType: "json"
		},
		...
		{insuranceName: 'dt-center',"width": "10px" ,
...
//	//alert(id);
    ...
    $.ajax({
        type: 'POST',
        url:'insurance/process.php',
		data: { action:'getdata_byid',ins_id:id} ,
        dataType: "json",
        success: function (data) {
          console.log(data.message);
            for (var i = 0; i < data.message.length; i++) {
                var counter = data.message[i];
         		$("#txtins_name").val(counter.ins_name);
				$("#txtins_name_en").val(counter.ins_name_en);				
				$("#txtins_domain").val(counter.ins_domain);
				$("#txtins_domain_en").val(counter.ins_domain_en);				

				$("#txtins_mobile1").val(counter.ins_mobile1);
				$("#txtins_mobile2").val(counter.ins_mobile2);				
				$("#txtins_mail").val(counter.ins_mail);
				$("#txtins_website").val(counter.ins_website);				
				$("#txtins_loc").val(counter.ins_loc);
				$("#cmbgov_id").val(counter.gov_id);				
								
				$("#cmbser_id").val(counter.ser_id);
				
				
				$("#hfins_id").val(counter.ins_id);
				$("#action").val("edit");
...
//====================================fileadd
				img = "";
				if(counter.ins_thumbnail != "")
				{
					img = "data-default-file=" +
					"../images/insurance/" + counter.ins_thumbnail;
				}
				...
				drEvent.on('dropify.beforeClear', function(event, element) {
					...
					if ($("#hfins_id").val() != "") {
						delimage("ins_image" , "ins_thumbnail");
					}
				  });

//====================================fileaddhover
				img1 = "";
				if(counter.ins_thumbnail1 != "")
				{
					img1 = "data-default-file=" +
					"../images/insurance/" + counter.ins_thumbnail1;
				}
				...
				drEvent1.on('dropify.beforeClear', function(event, element) {
					...
					if ($("#hfins_id").val() != "") {
						delimage("ins_image1" , "ins_thumbnail1");
					}
				  });
...
/*drEvent1 = $('#fileaddhover').dropify();
				...
				drEvent1.settings.defaultFile = "../images/insurance/" + counter.ins_thumbnail1;
				drEvent1.destroy();
				drEvent1.init();
				$('.dropify#fileaddhover').dropify({
				defaultFile: "../images/insurance/" + counter.ins_thumbnail1,
				});*/
//======================================
...
{
	 	_ins_name = $("#txtins_name").val();	
	_ins_name_en = $("#txtins_name_en").val();	
	_ins_mail = $("#txtins_mail").val();	
	_ins_mobile1 = $("#txtins_mobile1").val();	
	_ins_mobile2 = $("#txtins_mobile2").val();	
	_ins_loc = $("#txtins_loc").val();	
	_gov_id = $("#cmbgov_id").val();	
	if($.trim(_ins_name) == "" || $.trim(_ins_name_en) == ""
		|| $.trim(_ins_mail) == "" || $.trim(_ins_mobile1) == ""
		|| $.trim(_ins_mobile2) == ""|| $.trim(_ins_loc) == ""
		|| $.trim(_gov_id) == "")
...
function  editData()
{
    ...
    success: function (data) {
		//alert(data);
		/*$("#tdins_name" + _ins_id).html(_ins_name);
		$("#tdins_name_en" + _ins_id).html(_ins_name_en);		
...
function  delimage(_col1 , _col2)
{
    ...
    url: 'insurance/process.php',
    type: 'POST',
    data: { action:'delimage',ins_id:$("#hfins_id").val() , 
...
function  del(_ins_id)
{
    ...
    url: 'insurance/process.php',
    type: 'POST',
    data: { action:'delete',ins_id:_ins_id} ,
...
//=======================================================end delete 
//========================================= active
function insurance_active(_ins_id) {
	active = $("#active_" + _ins_id).attr("data_active");
	$.ajax({
    url: 'insurance/process.php',
    type: 'POST',
    data: { action:'update_active', ins_id:_ins_id ,
	 ins_active:active} ,
...
//		//alert(data);
		...
		{
			$("#active_" + _ins_id).attr("data_active", "on")
			$("#active_" + _ins_id).html("<img src = '../icon/true.gif'>");
		}
		else
		{
			$("#active_" + _ins_id).attr("data_active", "off")
			$("#active_" + _ins_id).html("<img src = '../icon/false.gif'>");
		}
    ...
    function active_all()
   {
		...
		else
		{
			...
			$('input[type=checkbox]').each(function () {
				...
				{
					insurance_active($(this).attr("check_value") );
				}
...
function fillgov()
{
 const selectElement = $('#cmbgov_id');
    ...
    $.ajax({
      url: 'insurance/process.php', // Replace 'get_options.php' with your PHP script URL
      type: 'POST',
      data: { action: 'getgov' }, 
      dataType: 'json',
...
function fillTypeCar()
{
 const selectElement = $('#cmbtyp_car_id');

    // Make an AJAX request
    $.ajax({
      url: 'insurance/process.php', // Replace 'get_options.php' with your PHP script URL
      type: 'POST',
      data: { action: 'getTypeCar' }, 
      dataType: 'json',
      success: function(response) {
        populateSelect(response);
      },
...
function fillTypeDriver()
{
 const selectElement = $('#cmbtyp_driver_id');

    // Make an AJAX request
    $.ajax({
      url: 'insurance/process.php', // Replace 'get_options.php' with your PHP script URL
      type: 'POST',
      data: { action: 'getTypeDriver' }, 
      dataType: 'json',
      success: function(response) {
        populateSelect(response);
      },
...
function fillTypeInsurance()
{
 const selectElement = $('#cmbtyp_ins_id');

    // Make an AJAX request
    $.ajax({
      url: 'insurance/process.php', // Replace 'get_options.php' with your PHP script URL
      type: 'POST',
      data: { action: 'getTypeInsurance' }, 
      dataType: 'json',
      success: function(response) {
        populateSelect(response);
      },
